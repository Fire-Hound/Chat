...
export default class App extends React.Component {
  ...
  sendText = async() => {
    ...
    responseJson = await POST("http://192.168.0.7:5000/", dataToSend)
    this.setState({
      currentText: "",
  ...
  update = async() =>{
    responseJson = await GET("http://192.168.0.7:5000/", 
  ...
  renderChatBubble = ({item}) => <ChatBubble text={item.text} user={item.user=="vikram"}/>
